...
def compare_text_by_characters(reference_text, user_text):
    ...
    print("\n--- 글자 단위 비교 ---")
    print(f"표준 발음: {' | '.join(reference_chars)}")
    print(f"사용자 발음: {' | '.join(user_chars)}")
    print(f"일치하는 글자 수: {match_count}/{total_chars}")
...
def compare_text_by_syllables(reference_text, user_text):
    ...
    print("\n--- 음절 단위 비교 ---")
    print(f"표준 발음: {reference_text}")
    print(f"사용자 발음: {user_text}")
    print(f"음절 단위 유사도: {ratio:.2f}%")
...
def compare_jamo_dtw(reference_text, user_text):
    ...
    # 자음/모음 리스트를 numpy 배열로 변환 (DTW 입력)
    ...
    # 결과 출력
    print("--- 자음/모음 DTW 비교 결과 ---")
    print(f"표준 발음 (자음/모음): {ref_jamo}")
    print(f"사용자 발음 (자음/모음): {user_jamo}")
    print(f"DTW 거리: {dist}")
...
def compare_syllables(reference_text, user_text):
    ...
    # 표준 발음의 음절을 하나씩 비교하면서, 사용자 발음에서 누락된 음절 찾기
    ...
    print("\n--- 음절 단위 비교 ---")
    print(f"표준 발음: {''.join(ref_syllables)}")
    print(f"사용자 발음: {''.join(user_syllables)}")
    if missing_syllables:
        print(f"누락된 음절: {''.join(missing_syllables)}")
    else:
        print("모든 음절이 발음되었습니다.")
def lcs(X, Y):
    m = len(X)
    n = len(Y)
